...
import { setSearch } from "../slices/NotificationUser";
...
export const load_tweet = () => async (dispatch) => {
  ...
  try {
    ...
    if (localStorage.getItem("access")) {
      res = await axiosInstance.get(`${url}tweets/`);
    } else {
...
export const explore_tweet = () => async (dispatch) => {
  ...
  try {
    let res;
    if (localStorage.getItem("access")) {
      res = await axiosInstance.get(`${url}tweets/explore/global/`);
    } else {
...
export const bookmark_list = () => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const res = await axiosInstance.get(`${url}tweets/love/bookmarkList/`);
    dispatch(setLoading(false));
...
export const tweet_specific_user = (username) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const res = await axiosInstance.get(`${url}tweets/specific/${username}/`);
    dispatch(setLoading(false));
...
export const addTweet = (uploadData) => async (dispatch) => {
  ...
  try {
    const res = await axiosInstance.post(`${url}tweets/`, uploadData);
...
export const reTweet = (tweetId) => async (dispatch) => {
  try {
    const res = await axiosInstance.post(`${url}tweets/post/retweet/`, {
      tweetId: tweetId,
    });
...
export const deleteTweet =
  ...
  async (dispatch) => {
    try {
      await axiosInstance.delete(`${url}tweets/${pk}/`);
...
export const editTweet = (id, title, isChecked) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const res = await axiosInstance.put(`${url}tweets/${id}/`, {
      title,
...
export const likeTweet = (id) => async (dispatch) => {
  try {
    const res = await axiosInstance.post(`${url}tweets/love/like-unlike/`, {
      pk: id,
    });
...
export const bookmarkTweet = (id) => async (dispatch) => {
  try {
    const res = await axiosInstance.post(`${url}tweets/love/bookmark/`, {
      pk: id,
    });
    ...
    if (query.length > 0) {
      if (isAuthenticated) {
        const res = await axiosInstance.get(
          `${url}tweets/search/custom/?search=${query}`
        );
        dispatch(setSearch(res.data));
      } else {
        const res = await axios.get(`${url}tweets/search/custom/?search=${query}`);
        dispatch(setSearch(res.data));
      }
    } else {
      dispatch(setSearch([]));
