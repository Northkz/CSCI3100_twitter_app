...
export const load_more_comment = (id, nextPage) => async (dispatch) => {
  try {
    const res = await axios.get(
      `${url}tweets/comments/${id}/?page=${nextPage}`
    );
...
export const addComment =(id, body, comid, reply = false) =>
  async (dispatch) => {
    ...
    try {
      ...
      if (reply) {
        const res = await axiosInstance.post(`${url}tweets/comments/reply/${id}/`, {
          body,
          comId: comid,
        });
        ...
        const res = await axiosInstance.post(`${url}tweets/comments/${id}/`, {
          body,
        });
        dispatch(commentUploading(false));
...
export const delComment = (id) => async (dispatch) => {
  try {
    await axiosInstance.delete(`${url}tweets/comment_detail/${id}/`);
...
export const likeComment = (id) => async (dispatch) => {
  try {
    const res = await axiosInstance.post(`${url}tweets/love/like-unlike-comment/`, {
      pk: id,
    });
...
export const editComment = (id, body) => async (dispatch) => {
  try {
    await axiosInstance.put(`${url}tweets/comment_detail/${id}/`, {
      body,
      isEdited: true,
    });
