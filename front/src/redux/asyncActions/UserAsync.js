...
export const userEdit = (username, data) => async (dispatch) => {
  // dispatch(setLoading(true));
  try {
    const res = await axiosInstance.put(`${url}user/${username}/`, data);
    dispatch(setLoading(false));
    dispatch(profileUserSuccess(res.data));
    dispatch(load_user());
...
export const userFollow = (username) => async (dispatch) => {
  try {
    const res = await axiosInstance.post(`${url}user/me/follow_unfollow/`, {
      username: username,
    });
...
export const load_more_user = (pageNum) => async (dispatch) => {
  try {
    const res = await axiosInstance.get(
      `${url}recommend_users/userlist/?page=${pageNum}`
    );
